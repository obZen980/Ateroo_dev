...
export class AterooKanbanGraph extends JournalDashboardGraphField {
    setup() {
        super.setup();
    ...
    getBarChartConfig() {
        ...
        return {
            ...
            options: {
                onClick: (e) => {
                    const agencyId = e.chart.config._config.options.agencyId;
                    if (!agencyId) {
                        return;
                    }
...
//                    TODO was taken from stock dashbord, please correct
                    const dateCategory = dateCategories[columnIndex];
                    const additionalContext = {
                        picking_type_id: agencyId,
                        search_default_picking_type_id: [agencyId],
                    };
...
//                    });
                ...
                agencyId: this.data[0].agency_id,
...
export const aterooKanbanGraph = {
    component: AterooKanbanGraph,
    supportedTypes: ["text"],
    extractProps: ({ attrs }) => ({
...
registry.category("fields").add("ateroo_kanban_graph", aterooKanbanGraph);
